...
public class SymTable {
    ...
    private Stack<Map<String, Set<Comparable>>> scopes;
    private List<Map<String, Set<Comparable>>> allScopes;
    ...
    public void popScope() {
        ...
        if (!this.scopes.isEmpty()) {
            Map<String, Set<Comparable>> currentScope = this.scopes.pop();
            // Guarda una copia del scope en la lista de todos los scopes
            this.allScopes.add(new HashMap<>(currentScope));
        }
    }
    
    public void add(String name, Set<Comparable> elements) {
        if (!this.scopes.isEmpty()) {
            this.scopes.peek().put(name, elements);
        }
    }
    
    public Set<Comparable> get(String name) {
    ...
    public void printAllScopes() {
        ...
        int scopeCounter = 0;
        for (Map<String, Set<Comparable>> scope : allScopes) {
